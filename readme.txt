...
2022-09-02 (test only) требуется обновить RESDUMP.BIN
...
2022-09-17 (test only)
	-Восстановление печати после выключения питания. Этап 2, проверка достаточности.
		по сигналу на PA2 или по переходу на паузу в каталоге создается файл RECOVERY.GCO (ранее файл назывался с ошибкой RECOVER.GCO, не путать)
		Запуск этого файла приведет к восстановлению ранее прерванной печати.
Эксперимент:
	Запустить печать, через некоторое время нажать на паузу, выключить питание принтера
	Включить питание принтера, в каталоге SHUI найти файл RECOVERY.GCO, запустить на печать.
	
	Точно такой же эксперимент можно провести с сигналом на PA2 (подробно не тестировалось)		

2022-09-20 (dev) Требуется тестирование.
	-Мапинг портов: концевики, bltouch, сенсоры филамента, UPS, kill switch
	-Оборудование считается отключенным, если соответствующий порт не задан. Флажки "сенсор установлен" и т.д. игнорируются и впоследствии будут удалены.

2022-09-22 (dev) Требуется тестирование.
	-Инверсия датчика филамента. Применение изменений без перезагрузки.
	-БВ
