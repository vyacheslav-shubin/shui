...
2021-12-16 (test-only) Требуется обновление RESDUMP.BIN, SHUIWIFI.BIN (ESP32)
	-Телеграмм диск
...
2021-12-25  (test-only) Требуется обновление RESDUMP.BIN, SHUIWIFI.BIN (ESP32)
	-Яндекс диск. Можно подключить публичные папки.
		Публичные папки Яндекс диска именю адреса вида: https://disk.yandex.ru/d/IgGCwtuzoiksMw
	...
	-Телеграмм бот
		...
		Через телеграмм бота можно получить досту к публичным git репозиториям. GitHub позволяет взаимодействовать с ним анонимному пользователю через специальный сервис,
		но это взаимодействие ограничено лимитом обращения в месяц. Для снятия этих ограничений на git надо зарегистрироваться и по агалогии с Яндекс диском получить 
		...
		можно добавлять репозитории git с публичным доступом. Например: /git add vyacheslav-shubin/3d-models 
		...
		Точно так же можно регистрировать источники с Яндекс диска. Файлами ссылками к слючами, выданными ботом можно обмениваться, так как и отозвать их
