...
Полезные ссытки:
	...
	Графомания о SHUI (Владимир Шумов): https://telegra.ph/Grafomaniya-o-SHUI-11-09
	
2024.06.08
	Данный документ является логм разработки. Некоторые идеи описанные в его начале переосмыслены.
	В настоящий момент многие блоки Марлин заменены на собственные, что позволило внедрить новую функциональность, в частности маппинг портов.
	Существенному изменению подвергся код Марлин. Ядро GRBL оставлено практически без изменений.
...
2024-05-19 Требуется обновление RESDUMP
	...
	-📍Trinamic, доступен регистр состояния драйвера 0X6F: DRV_STATUS – DRIVER STATUS FLAGS AND CURRENT LEVEL READ BACK (5.5.3),
	позволяет определить перегрев драйвера, обрыв и замыкание обмоток, текущий режим работы StealthChop/SpreadCycle
	-📍M2019C - проверка состояния драйверов, вывод содержания регистра DRV_STATUS в консоль. M2019CH - дополнительная опция H - halt, остановка принтера в случае перегрева, обрыва, замыкания обмоток.
...
2024-06-08 Требуется обновление RESDUMP
	-📍Опция "В пределе" устанавливает ограничение на перемещение за пределы размера оси. Ограничиваются только запаркованные оси. Отключенная опция перекрывает управление M211, и программные концевики остаются всегда выключены.
		С этого обновления ограничение действет на запаркованную ось, в отличии от предыдущих версий, когда ограничение включалось только если все оси были запаркованы.
	-📍Настройки Ретрактов, перекоса, люфта, гашения вибраций перенесено в подраздел Коррекция
	-📍Настройки Телеграм перенесено в подраздел Конфигурация WiFi
	-📍Если ось запаркована, долгое нажатите на кнопку парковки (ось или все), приводит к перемещению по оси (осям) в координату 0
	-📍Конфигурация UI опция "Ввод координат. Установка/перемещение". 
		Установка (SET), координата примет введенное значение, перемещение не произойдет
		Перемещение (MOVE),  координата примет введенное значение, каретка сопла будет перемещена.❗️ВНИМАНИЕ, контрль координат не выполнятется. Если вы зададите значение за пределами оси, так тому и быть.
	-📍Долгое нажатие на селектор оси в виджете перемещения позовляет ввести кординату (перемещение или установка)
	-📍Сервис "телеметрия" отправляет сообщения в телеграмм изменяя ранее отправленное - альтернатива большому потоку сообщений.
	-📍Сервис "телеметрия" создает и поддерживает состояние устройств Яндека Умный дом. Создаются устройства: Термостабилизаторы стол и сопло, счетчик общего расхода филамента, выключатель принтера. Устройства могут управлять состоянием принтера, 
	но для этого должен быть установлен и настроен дополнительный телеграмм бот. 
	-📍Информация о WiFi добавлен MAC адрес модуля, состояние сервиса Телеметрия.
	-📍Модуль Esp32 перезагружается после окончания загрузки файла.


=========================== test only	
		

❗️❗️❗️❗️❗️❗️❗️❗️❗️❗️❗️❗️❗️❗️❗️❗️ Внимание, начиная с этого обновления, предыдущие прошивки модуля WiFi ESP32 могут оказаться несовместимы. Модуль ESP32 обновлять обязательно, если это указано в примечании к выпущенной версии. Прошивка модуля ESP32 имеет ограничение на время использоваия - 30 дней со дня публикации. По истечении этого врмени функции телеметрии и бота управления принтером будут отключены. После получения достаточного количества откликов о работе ограничение на время использвания сниму.
		

2024-07-16 (test_only) Требуется обновление WIFI модуля ESP32
	-📍Переключатель сообщения "напрямую в телеграмм"/"Обработка сервером телеметрии" (рекомендуется)
	-📍Диагностический виджет состояния модуля WiFi отображает ID бота пользователя - бота управления принтером
	-📍Виджеты конфигурирования не скрывают кнопок навигатора на первой и последней странице.
	-📍Если отправить файла в TG канал связанный с принтером с подписью "print" файл будет отправлени на печать (исполнен), "save" - сохранен,
		 "link" на флеш карте будет создан файл-ссылка позволяющий в дальнейшем загрузить файл (не полностью доступно).
	-📍Ограничение DEV версии модуля WiFi - 30 дней
	-📍https://telegra.ph/Shubot-06-12

2024-07-21 (test_only) Требуется обновление WIFI модуля ESP32
	-📍Управление звуковым сигналом. Добавлены новые параметры для команды M300. U - время звучания (мс), D - время паузы (мс), R - количество  повторений. Следует учесть, что в отличии от M300P комада является синхронной. Это означает, что пока она выполняется, принтер не будет реагировать на другие команды или управление через UI. Встроенный в прошивку сигнал критической ситуации можно задать так: M300 U25 D80 R20 или M300A
	M300 U100 D100 R3
	M300 U200 D100 R3
	M300 U100 D100 R3

2024-08-03 (test_only) Требуется обновление WIFI модуля ESP32, RESDUMP.BIN
	-📍Если отправить файла в TG канал связанный с принтером с подписью "print" файл будет отправлен на печать (исполнен), "save" - сохранен,
		 "link" на флеш карте будет создан файл-ссылка позволяющий в дальнейшем загрузить файл. Таким образом каталогизация файлов канала телеграмм ведется на флеш карте принтеора.
		 Для того, чтобы файлы c расширением tgl стали видимы, расширьте фильтр видимых файлов в настройках виджета браузера файлов.	
	-📍Ограничение DEV версии модуля WiFi - 30 дней
	-📍Опция автоматический babystepping. Если в процессе печати использовался babystepping, по окончании печати будет автоматически выполнена корректировка dZ сенсора стола или концевика Z. (спасибо, Виктор, за идею)

2024-09-22 (test_only)
	-📍Поддержка бутлоадера FWBL (Внимание❗️❗️❗️ Прошивка SHUI не содержит алгоритмов замены бутлоадера. Инструкцию по замене ищите у автора. В случае возникновения критической ошибки при замене бутлоадера вам понадобится дополнительное оборудование для низкоуровневой прошивки контроллера)
	-📍Параметр "Температура замены филамента" выведен в настройки
	-📍M808
	-📍Ограничение DEV версии модуля WiFi - 90 дней (SHIWIFI.BIN)
	
2024-11-21 (test_only)
	- Виджет перемещения. Адаптация для лазера, фрезера
	- Сохраненная сетка уровней стола включается в общий бакап
	- Поддержка плат Nano6
	- Для принтеров с двумя асинхронными моторами оси Z и bltouch добавлен алгоритм выравнивания горизонта стола без дополнительных концевиков по Z
		❗️Текущая реализация сначала делает замер 2х сторон стола, затем коррекцию перекоса, поэтому при замере стороны, которая ниже, другая сторона может подняться выше плоскости сопла, следует это учесть, если установлены физические ограничители осей.
		Параметром расстояние Z1-Z2 может быть подобрана точность измерения. В качестве начального значения следует установить межосевое расстояние винтов осей Z1 и Z1. ❗️Важно❗️ В настройках портов для Z1 должен быть указан мотор расположенный ближе к началу координат.


2025-01-25 (test_only)
	- Добавлен новый набор параметров, определяющих задержку после активаци/деактиванции сенсора стола (мс). Для bltouch значения добавляются к рекомендуемым 750мс.
	- Восстановление из бакапа назначения порта PA0 (исправлена ошибка)
	- Поддержка сенсора стола MicroProbe
	- Дублирование моторов по осям X и Y, для моторов X2, Y2 можно отдельно указать значение инверсии.
	- Функция "Толчок" при выгрузке филамента. Перед выгрузкой выполняется короткий толчок вперед (настраивается)
	- Дискреты перемещения в виджетах "Перемещение", "Филамент" можно настроить.

2025-01-27 (test_only)
	- Зациклившееся самотестирование bltouch, попытка исправить. Проверьте, у кого проявлялось.
	- Параметр настроек алгоритма замера сетки Z min теперь соответствует уровню, до которого будет двигаться стол, во время замера сетки. Ранее этот параметр означал уровень относительно предполагаемой точки срабатывания сенсора. 
	Таким образом Z-min и Z-max определяют диапазон движения стола относительно ноля во время замера сетки. Рекомендуемое значение этого параметра [0..1]. Ранее сделанные настроqки с отрицательными значениями не будут иметь критического влияния, при стабильной работе сенсора.

2025-02-28 (test_only)
	- Исправлена ошитбка отображеия состяния i2c устройств в виджете WiFi.
	- Параметр настроек алгоритма замера сетки Z min теперь соответствует уровню, до которого будет двигаться стол, во время замера сетки. Ранее этот параметр означал уровень относительно предполагаемой точки срабатывания сенсора. 
	Таким образом Z-min и Z-max определяют диапазон движения стола относительно ноля во время замера сетки. Рекомендуемое значение этого параметра [0..1]. Ранее сделанные настройки с отрицательными значениями не будут иметь критического влияния, при стабильной работе сенсора.
	- Расширены возможности маппинга портов. Порты используемые модулем WiFi можно использовать по желанию для других устройств, если модуль не установлен.
	- Управление сенсором bltouch (не путать с датчиком сенсора), может быть назначен на любой порт. Учитывайте, что управление bltouch и servo должны быть назначены на порт имеющий аппаратный PWM (например, A8, RX3, TX3). Внимание ❗️❗️❗️ на плате RN6 подписи пинов порта UART3 перепутаны. Если сенсор bltouch не используется, порт A8 - хороший выбор для управления мощностью лазера.
	- Для модулей WiFi, у который "выломан зуб" и соединены 2 пина вместе (вариант подключения к RN11 совместрно с bltouch) назначение WiFi-PROG может быть выставленно в OFF. Для тех, кому не хватает свободных портов (RN6 порт для управления выключением), можно использовать освободившийся дополнительный порт, доработав модуль WiFi
	- RN6 установленный разъем USB работает, можно использовать USB флешки.
Доработки значительные, поэтому, прошу тех, кто установил дать обратную связь о тестировании.	
	
	
2025-04-06 (test_only), требуется обновление RESDUMP.BIN
	-📍M109, M190 Дополнительная опция U(0|1), При установленном значении 1 - отключается алгоритм ожидания вхождения температуры в диапазон +-1 градус. Температура считается достигнутой, если текущее значение просто превысит целевое.
	-📍Bang-Bang значение гистерезиса может быть указано с долями градуса. Гистерезис определяет только нижнюю границу значения температуры. Верхняя граница - целевая температура
	-📍"FWBL bootloader" доступен только только мастерам
	-📍Исправлена ошибка инициализации bltouch. Проявлялась на платах RN12 в виде "гудящих" моторов. Эффект пропадал при повторной инициализации bltouch
	-📍M0 дополнительные параметры
		P - пауза в миллисекундах
		S - пауза в секундах
		M - пауза в минутах.
		Общая длительность паузы P + S*1000 + M * 60 * 1000
		B - блокирующий вызов или только предварительные настройки. По умолчанию - блокирующий вызов.
		R - запуск ранее сконфигурированного ожидания
		Пример:
			M0 M90 B0       		; Конфигурирование ожидания M0 - 90 минут
			....
			....
			M0R Сушим пластик		; Запуск ранее сконфигурированной паузы
	-📍Механизм триггеров и действий
		Для нагревателей (в настоящий момент в качестве источников сигнала могут быть использованы только нагреватели), можно задать действия, которые будут выполнены если значение температуры превысило или стало ниже заданного значения
		Пример:
		M2036 I0                ; Триггер 0 (стол). Сброс состояния. Нижний порог и верхний порог - действие по-умолчанию (бипер по выходу за границы)
		M2036.1 I0 E L2 H2      ; Настройка триггера 0, разрешен, нижний порог -2 от целевой, верхний +2 от целевой
		
		Действия для триггера могут быть переопределены. Подробнее см. gcode_info.txt
		...
		M2036.1 I0 E A L78 H82    ;Собственные настройки триггера температуры стола. Триггер активен. Порог выключения - 78 градусов, порог включения 82. Пороги абсолютные
	-📍Виджет Cooking - расширенная реализация Йогурт таймера. С использованием описанных выше механинзмов могут быть созданы управляющие программы разного назначения
		Запуск виджета из г кода:
		M2006 N33
		
		Пример файла cooking.SNI
		
		M2006 N33       ; Включить виджет Cooking
		M0 M90 B0       ; Конфигурирование (только конфигурирование) ожидания M0 - 90 минут
		M190 S120 UM    ; Ожидание нагрева до 120 без стабилизации Unstable, Muted
		M300            ; Beep

		M2036 I0                ; Триггер 0 (стол). Сброс состояния. Нижний порог - действие 0, верхний порог - действие по-умолчанию (бипер по выходу за границы)
		M2036.1 I0 E L2 H2      ; Настройка триггера 0, разрешен, нижний порог -2 от целевой, верхний +2 от целевой

		M0R Cooking               ; Запуск ранее сконфигурированного ожидания
		M140 S0
		M117 Done
		M300        ; Beep

2025-06-01 (test_only), требуется обновление RESDUMP.BIN
	-📍Исправлен алгоритм отката после парковки, если для концевых датчиков указано отрицательное смещение
	-📍В режиме настройки фокусировки лазера. После перемещения лазер не выключается. Во время режима фокусировки можно добавить мощность.
	-📍Прокрутка в браузере файлов теперь циклическая. После последней страницы переход на первую и наоборот. Долгое нажатие на кнопки перелистывания страниц выполняет соответствующий переход либо к первой странице, либо к последней.
	-📍Опция "Запрет старта УП без парковки" (настройки UI). Не позволяет запустить управляющую программу, если координаты X или Y не определены. Необходимо выполнить парковку или задать точку ноля.

2025-06-22 (test_only), требуется обновление RESDUMP.BIN
	-📍RN6 - поддержка управления лазером
	-📍Лазер может быть подключен к PA8 вместо bltouch
	-📍Обновлен виждет преднастроек перед запуском задания для лазера.
	-📍Для лазера добавлен трансформатор g-кода. Можно переопределить значение S(pwm). В текущей реализации S0 - остается без замены, S[n>0] - заменяется на заданное значение.
	-📍Для PWM лазера можно указать число ступеней. По умолчанию 256, можно задать 1000.
	 
2025-07-13 (test_only), требуется обновление RESDUMP.BIN
	-📍PA8 можно использовать для управления лазером на всех платах
	-📍Пресеты скорости перемещения. Алгоритм работает только для конфигурации Лазер или Гравер. Включается в разделе конфигурации "G код".
		Если при формировании G кода скорость подачи указать как F1, F2, F3 или F4, трансформатор G кода заменит эти значения на соответствующие из заранее определенной таблицы замены. 
		Таблица замены может быть отредактирована непосредственно перед запуском файла управляющей программы, или определена заранее в конфигурации (раздел конфигурации "G код")
	
2026-03-22 (test_only) обновление для ESP32, только SHUIWIFI.BIN
	-📍Убрана проверка доступлости телеграмм. В условиях блокировки приводила к критической ошибке и модуль перезапускался.

https://gitverse.ru/vyacheslav-shubin/shui/content/master/test-only
