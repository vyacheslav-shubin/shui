...
2025-01-27 (test_only)
	...
	- Параметр настроек алгоритма замера сетки Z min теперь соответствует уровню, до которого будет двигаться стол, во время замера сетки. Ранее этот парамет означал уровень относительно предполагаемой точки срабатывания сенсора. 
	Таким образом Z-min и Z-max определяют диапазон движения стола отностительно ноля во время замера сетки. Рекомендуемое значение этого параметра [0..1]. Ранее сделанные настроки с отрицательными значениями не будут иметь критического влияния, при стабильной работе сенсора.
