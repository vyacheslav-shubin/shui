...
printers = [
  {'name':'192.168.2.24', 'ip':'192.168.2.24', "esp32":True}
]
...
class NetOutputDialog(QDialog):
  def __init__(self):
    ...
    edFileName = QLineEdit()
    edFileName.setText(self.getFileName())
    edFileName.setMaxLength(32)
    file_name_layout.addWidget(edFileName, alignment=QtCore.Qt.AlignLeft)
    self.edFileName=edFileName
    file_name_layout.addWidget(QLabel(".gcode"), alignment=QtCore.Qt.AlignLeft)
  ...
  def onOk(self):
    ...
    file_name=self.edFileName.text() + '.gcode';
    try:
